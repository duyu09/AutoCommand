...
def ProcessLang(content):
    # 自然语言预处理，使之更好的适于本项目。
    content = content.replace("关机","关闭计算机")
    content = content.replace("调亮屏幕" ,"调高屏幕亮度")
    content = content.replace("调暗屏幕" ,"调低屏幕亮度")
    content = content.replace("屏幕调亮" ,"调高屏幕亮度")
    content = content.replace("屏幕调暗" ,"调低屏幕亮度")
    content = content.replace("调大声音" ,"调高系统音量")
    content = content.replace("调小声音" ,"调低系统音量")
    content = content.replace("声音调大" ,"调高系统音量")
    content = content.replace("声音调小" ,"调低系统音量")
    content = content.replace("调高声音" ,"调高系统音量")
    content = content.replace("调低声音" ,"调低系统音量")
    content = content.replace("声音调高" ,"调高系统音量")
    content = content.replace("声音调低" ,"调低系统音量")
    content = content.replace("点击","单击")
    content = content.replace("点开","单击")
    content = content.replace("开开","双击")
    content = content.replace("打开","双击")
    content = content.replace("点击","单击")
    return content
...
if __name__ == '__main__':
    ...
    start = time.time()
    if not os.path.exists(os.getenv("temp") + r"\AutoCommand_Output.tmp"):
        print("ERROR")
    filein = open(os.getenv("temp") + r"\AutoCommand_Output.tmp", 'r', encoding='utf-8')
    # content="现在正在运行指令，请打开浏览器。"
    content = filein.readline()
    filein.close()

    events, spos = handler.phrase_ip(content)
    # spos = [i for i in spos if i[0] and i[2]]
    tempfile = open(os.getenv("temp") + r"\AC_Command.tmp", 'w')
    spos = [i for i in spos]
    for spo in spos:
        strq = ProcessLang(str(spo))
        tempfile.write(strq + "\n")
    tempfile.close()
